...
void* march(void* arg) {
	...
	// Get the arguments from the structure
	...
	int start = thread_id * (double)p / num_threads;
	int end = min((thread_id + 1) * (double)p / num_threads, p);
...
int main(int argc, char *argv[]) {
	...
	// printf("numarul de thread-uri pt acest task este %d \n", num_threads);
	...
	for (int i = 0; i < num_threads; i++) {
		thread_data[i].id = i;
		thread_data[i].image = scaled_image;
		...
		pthread_create(&threads[i], NULL, march, &thread_data[i]);
	}
